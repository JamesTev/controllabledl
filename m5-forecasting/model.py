...
class M5Net(nn.Module):
    def __init__(
        self, data_info, rule_encoder, data_encoder, name_to_ind, hidden_dim=16
    ):
    ...
    def forward(self, x, alpha=0.0, merge="cat"):
        ...
        if merge == "add":
            ...
            z = torch.cat(
                (alpha * rule_z, (1 - alpha) * data_z), dim=-1
            )  # merge: Concat
...
class Net(nn.Module):
    ...
    def forward(self, dict_data):
        ...
        for key in self.cat_keys:
            emb_out.append(
                self.emb_layers[key](dict_data[key]).squeeze(1)
            )  # emb input shape: (batch_size, 1)
