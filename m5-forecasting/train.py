...
import utils
from model import DataEncoder, M5Net, RuleEncoder
from torch.distributions.beta import Beta
from torch.utils.data import DataLoader, TensorDataset
from utils_learning import get_perturbed_input, verification

week_dense_cols = [
    "week_sell_price",
    "diff_week_price",
    "week_sell_price_rel_diff",
    "week_sell_price_roll_sd7",
    "week_sell_price_cumrel",
    "week_lag_t28",
    "week_rolling_mean_t7",
    "week_rolling_mean_t30",
    "week_rolling_mean_t60",
    "week_rolling_mean_t90",
    "week_rolling_mean_t180",
    "week_rolling_std_t7",
    "week_rolling_std_t30",
]
...
def main():
    ...
    data_info = {
        "item_id": df_input["item_id"].unique().shape[0],
    ...
    valid_x_list = [
        valid_x["dense1"],
        valid_x["item_id"].squeeze(),
        valid_x["dept_id"].squeeze(),
        valid_x["store_id"].squeeze(),
        valid_x["cat_id"].squeeze(),
        valid_x["state_id"].squeeze(),
    ]
    test_x_list = [
        test_x["dense1"],
        test_x["item_id"].squeeze(),
        test_x["dept_id"].squeeze(),
        test_x["store_id"].squeeze(),
        test_x["cat_id"].squeeze(),
        test_x["state_id"].squeeze(),
    ]
    ...
    print(
        "model_type: {}\tscale:{}\tBeta distribution: Beta({})\tlr: {}\trule_coeff: {}\tpert_coeff: {}".format(
            model_type, scale, beta_param, lr, rule_coeff, pert_coeff
        )
    )
    ...
    model = M5Net(data_info, rule_encoder, data_encoder, name_to_ind, hidden_dim_db).to(
        device
    )  # Not residual connection

    pert_feature_ind = (
        0  # Index of the feature we impose perturbation. Index of price is 0.
    )
    ...
    saved_filename = (
        "m5_{}_weekly_demand_pred_with_pert_price-thld{}seed{}.demo.pt".format(
            model_type, corr_threshold, seed
        )
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            pert_batch_train_x[0][:, pert_feature_ind] = get_perturbed_input(
                pert_batch_train_x[0][:, pert_feature_ind], pert_coeff
            )
            pert_output = model(
                pert_batch_train_x, alpha=alpha, merge=merge
            )  # \hat{y}_{p}    predicted sales from perturbed input

            loss_rule = loss_rule_func(
                pert_output, output
            )  # pert_output should be less than output
        ...
        if epoch % valid_freq == 0:
            ...
            with torch.no_grad():
                ...
                pert_valid_x[0][:, pert_feature_ind] = get_perturbed_input(
                    pert_valid_x[0][:, pert_feature_ind], pert_coeff
                )
                pert_output = model(
                    pert_valid_x, alpha=alpha, merge=merge
                )  # \hat{y}_{p}    predicted sales from perturbed input
            ...
            if val_loss < best_val_loss:
                ...
                print(
                    "[Valid] Epoch: {} Loss: {:.6f} (alpha: {:.2f})\t Loss(Task): {:.6f}\t Loss(Rule): {:.6f}\t Ratio: {:.4f} best model is updated %%%%".format(
                        epoch,
                        best_val_loss,
                        alpha,
                        val_loss_task,
                ...
                print(
                    "[Valid] Epoch: {} Loss: {:.6f} (alpha: {:.2f})\t Loss(Task): {:.6f}\t Loss(Rule): {:.6f}\t Ratio: {:.4f}({}/{})".format(
                        epoch,
                        val_loss,
                        alpha,
                        val_loss_task,
    ...
    data_encoder = DataEncoder(input_dim, output_dim_encoder, hidden_dim_encoder)
    model_eval = M5Net(
        data_info, rule_encoder, data_encoder, name_to_ind, hidden_dim_db
    ).to(
        device
    )  # Not residual connection
    ...
    pert_test_x[0][:, pert_feature_ind] = get_perturbed_input(
        pert_test_x[0][:, pert_feature_ind], pert_coeff
    )
    for alpha in alphas:
        model_eval.eval()
        with torch.no_grad():
            ...
            if model_type.startswith("dataonly"):
                pert_output = model_eval(
                    pert_test_x, alpha=0.0, merge=merge
                )  # \hat{y}_{p}    predicted sales from perturbed input
            elif model_type.startswith("ours"):
                pert_output = model_eval(
                    pert_test_x, alpha=alpha, merge=merge
                )  # \hat{y}_{p}    predicted sales from perturbed input
            elif model_type.startswith("ruleonly"):
                pert_output = model_eval(
                    pert_test_x, alpha=1.0, merge=merge
                )  # \hat{y}_{p}    predicted sales from perturbed input
