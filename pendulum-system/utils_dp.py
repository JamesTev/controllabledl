...
class DoublePendulum:
    def __init__(self, L1, L2, M1, M2, theta1, omega1, theta2, omega2, F1=0.1, F2=0.1):
        """
    ...
    def generate(self, tmax, dt, energy_tol=0.01):
        ...
        print(
            "Length (L1,L2), Mass (M1,M2), and "
            "Friction (F1,F2) of a string: ({},{}) ({},{}), ({},{})".format(
                self.L1, self.L2, self.M1, self.M2, self.F1, self.F2
            )
        )
        print(
            "Initial theta(degree): {:.6f}({:.6f}),{:.6f}({:.6f})".format(
                y0[0], 180.0 * y0[0] / np.pi, y0[2], 180.0 * y0[2] / np.pi
            )
        )
    ...
    def deriv_double(self, y, t):
        ...
        C1 = (
            -M2 * L2 * z2 ** 2 * np.sin(theta1 - theta2)
        ...
        C2 = (
            M2 * L1 * z1 ** 2 * np.sin(theta1 - theta2)
            - M2 * g * np.sin(theta2)
            - F2 * L2 * z2
        )
...
def calc_double_E(y, **kwargs):
    ...
    if isinstance(y, np.ndarray):
        ...
        V = (
            -(M1 + M2) * L1 * g * torch.cos(th1)
            - M2 * L2 * g * torch.cos(th2)
            + M1 * g * L1
            + M2 * g * (L1 + L2)
        )
        T = 0.5 * M1 * (L1 * th1d) ** 2 + 0.5 * M2 * (
        ...
        raise TypeError(
            "type of y is :{}. It should be numpy.ndarray or torch.Tensor".format(
                type(y)
            )
        )
