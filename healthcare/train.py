...
from model import DataEncoder, Net, RuleEncoder
...
def main():
    ...
    print(
        "# of cardio=1: {}/{} ({:.2f}%)".format(
            np.sum(y == 1), len(y), 100 * np.sum(y == 1) / len(y)
        )
    )
    print(
        "# of cardio=0: {}/{} ({:.2f}%)\n".format(
            np.sum(y == 0), len(y), 100 * np.sum(y == 0) / len(y)
        )
    )
    ...
    low_ap_negative = (df[rule_feature] <= rule_threshold) & (
        df["cardio"] == 0
    )  # usual
    high_ap_positive = (df[rule_feature] > rule_threshold) & (
        df["cardio"] == 1
    )  # usual
    low_ap_positive = (df[rule_feature] <= rule_threshold) & (
        df["cardio"] == 1
    )  # unusual
    high_ap_negative = (df[rule_feature] > rule_threshold) & (
        df["cardio"] == 0
    )  # unusual
    ...
    num_samples_from_usual = int(
        num_samples_from_unusual * src_usual_ratio / (1 - src_usual_ratio)
    )

    X_src = np.concatenate(
        (X_usual[:num_samples_from_usual], X_unusual[:num_samples_from_unusual]), axis=0
    )
    y_src = np.concatenate(
        (y_usual[:num_samples_from_usual], y_unusual[:num_samples_from_unusual]), axis=0
    )
    print()
    ...
    print(
        "Usual ratio: {:.2f}%".format(100 * num_samples_from_usual / (X_src.shape[0]))
    )
    ...
    train_X, test_X, train_y, test_y = train_test_split(
        X_src, y_src, test_size=1 - train_ratio, random_state=seed
    )
    ...
    saved_filename = "cardio_{}_rule-{}_src{}-target{}_seed{}.demo.pt".format(
        model_type, rule_feature, src_usual_ratio, src_usual_ratio, seed
    )
